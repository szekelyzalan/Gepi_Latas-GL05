...
from utils.categories_to_keep import CLASS_COUNTS
...
def get_expected_cnt() -> dict[int, dict[str, int]]:
    """
    Processes categories_to_keep.txt.
    """
    exp_cnt = defaultdict(dict)
    for i, label in enumerate(CLASS_COUNTS):
        exp_cnt[i] = {
            "all": CLASS_COUNTS[label],
            "train": 0,
            "val": 0,
            "test": 0
        }
